...
data class Airline(
    ...
    /**
     * 캠페인 내내 증자로 찍어낸 신주 총합. 평생 희석 한도의 기준이고,
     * `shares - issuedTotal` 이 곧 증자 전 원래 주식 수다.
     *
     * 이게 없으면 방어측이 분기마다 30% 를 무한히 찍어낼 수 있다. 매집이 분기 10% 이므로
     * 지분율이 (f+0.1)/1.3 의 고정점인 33% 에서 수렴해 — 적대적 인수가 **산술적으로**
     * 불가능해진다. 실제로 플레이에서 그 증상이 나왔다.
     */
