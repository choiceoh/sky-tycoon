...
object Events {
    ...
    /**
     * 일시 수요 효과를 건다. 이미 걸린 효과를 **덮어쓰지 않고 나란히 쌓는다** —
     * 사스로 0.45 까지 떨어진 홍콩에 무작위 관광 붐이 덮이면 폭락이 통째로 사라지고,
     * 하나로 합쳐 두면 이번엔 짧은 효과가 긴 효과의 만료까지 살아남는다.
     */
    private fun GameState.cityBoost(cityId: String, mult: Double, quarters: Int): GameState {
        val cs = (cityState[cityId] ?: CityState()).pruned(turn)
        return copy(
            cityState = cityState + (
                cityId to cs.copy(effects = cs.effects + CityEffect(mult, turn + quarters - 1))
                ),
        )
    }
    ...
    fun stepWorld(state: GameState, rng: Rng): GameState {
        ...
        return s.copy(
            ...
            // 만료된 일시 효과는 여기서 털어낸다 — 안 그러면 20년치가 그대로 쌓인다.
            cityState = s.cityState.mapValues { (_, cs) -> cs.pruned(s.turn) },
        )
    }
